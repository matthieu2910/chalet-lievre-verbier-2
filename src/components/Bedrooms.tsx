...
// Fallback bedrooms if no CMS data is available
const fallbackBedrooms: BedroomData[] = [
  {
    name: "Master Suite",
  ...
  {
    name: "Alpine Suite",
    image: "https://images.unsplash.com/photo-1617325247661-675ab4b64ae2?q=80&w=2942&auto=format&fit=crop",
    bedType: "Queen Bed",
    guests: 2,
  ...
  {
    name: "Cozy Room",
    image: "https://images.unsplash.com/photo-1615874959474-d609969a20ed?q=80&w=2880&auto=format&fit=crop",
    bedType: "Queen Bed",
    guests: 2,
...
export function Bedrooms({ bedrooms }: BedroomsProps) {
  ...
  // Use provided bedrooms, or fallback if none available
  const bedroomsData = bedrooms && bedrooms.length > 0 ? bedrooms : fallbackBedrooms;
  ...
  return (
    <section id="bedrooms" ref={ref} className="section-padding bg-alpine-100/50">
      <div className="container-wide">
        ...
        {/* Bedrooms Grid */}
        {bedroomsData.length > 0 ? (
          <div className="grid md:grid-cols-2 gap-8">
            {bedroomsData.map((bedroom, index) => (
              <motion.div
                key={bedroom.name}
                initial={{ opacity: 0, y: 30 }}
                animate={isInView ? { opacity: 1, y: 0 } : {}}
